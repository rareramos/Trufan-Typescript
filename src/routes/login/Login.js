...
class Login extends React.Component {
  ...
  signUp = event => {
    ...
    xhr.addEventListener('load', () => {
      if (xhr.status === 200) {
        ...
        // this.handleLoginClick({ preventDefault: () => { } }); //unfortunate event workaround..
      } else {
        // failure
  ...
  login = event => {
    ...
    xhr.addEventListener('load', () => {
      if (xhr.status === 200) {
        ...
        // TODO: Currently after logging in, header is not updated... need to trigger update to navigation
        history.push({
          pathname: '/',
          state: { id_token: xhr.response.token },
        });
      } else {
        // failure
  ...
  sendLoginForm = formData => formData;
    ...
    return (
      <div className={s.root}>
        <div className={s.container}>
          ...
          <p className={s.lead}>
            Log in with your social media account or personal email address.
          </p>
          {/* <div className={s.formGroup}>
            <a className={s.facebook} href="/dashboard">
              <svg
                className={s.icon}
                width="30"
                height="30"
          ...
          <div className={s.formGroup}>
            <a className={s.google} href="/dashboard-yt">
              <svg
                className={s.icon}
                width="30"
                height="30"
