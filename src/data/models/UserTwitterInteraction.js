...
const UserTwitterInteraction = Model.define('UserTwitterInteraction', {
  tweet_id:{
...
const username = 'goswish';

const getAccountMentions = (username) => {
  client.get(
    'tweets/search/30day/trufanwebdev',
    {
      query: `@${username}`
    },
    (error, tweets) => {
      if (!error) {
        console.log(tweets);
        const ts = _(tweets.results)
          .map(tweet => {
          return { 
            tweet_id: tweet.id_str,
            twitter_handle: username,
            content: { 
              s: sentiment.analyze(tweet.text),
              text: tweet.text
            },
            twitter_data: tweet
          }
        });
        return UserTwitterInteraction.bulkCreate(ts).then(() => {
          console.log(`Added ${ts.length} interaction(s)`)
        })
      } else {
        console.log(error);
      }
...
const getAccountReplies = (username) => {
  client.get(
    'tweets/search/30day/trufanwebdev',
    {
      query: `@${username}`
...
const reverseLookupRecurse = (i, result, tpId) => {
  if (i >= 5) {
    ...
    return UserTwitterInteractions.update(
...
const startUserReverseLookup = () => {
  UserTwitterInteractions.findOne({
    where: {
...
const startNextTweet = () => {
  UserTwitterInteractions.findOne({ where: { got_tweets_time: null } }).then(
    twitterPassionate => {
      if (twitterPassionate) {
...
setIntervalAndExecute(getAccountMentions, RATE_LIMIT_SEARCH_MENTIONS);

export default UserTwitterInteraction;
